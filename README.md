...
`frame-me-parent` 是一个基于 **Spring Boot 4.0.7 + Java 25** 的多模块 Maven 项目，用于沉淀通用的基础设施、响应规范、异常处理与自动装配约定。项目通过分层模块设计，让外部应用可以选择只引入所需能力，或通过 `frame-me` 聚合模块一键引入全部通用能力。
...
| `frame-me-base` | Spring Web 基础设施：全局异常处理、自动装配。 |
| `frame-me-adapter` | 适配层：将内部 `Result<T>` 转换为外部 `Response<T>`。可被外部项目重写。 |
| `frame-me-auth` | 认证授权占位模块。 |
| `frame-me-cloud` | 微服务云组件占位模块。 |
| `frame-me` | 聚合模块，统一引入通用能力（不含 `frame-me-adapter`）。 |
