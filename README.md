...
`frame-me-parent` 是一个基于 **Spring Boot 4.0.7 + Java 25** 的多模块 Maven 项目，用于沉淀通用的基础设施、响应规范、异常处理与自动装配约定。项目通过分层模块设计，让外部应用可以选择只引入所需能力，或通过 `frame-me-booter` 聚合模块一键引入全部通用能力。
...
| 模块 | 定位 |
|---|---|
| `frame-me-api` | 接口契约模块：`IResult<T>` 接口、`ApiConstant`。 |
...
| `frame-me-booter` | 聚合模块，统一引入通用能力（不含 `frame-me-starter-adapter`）。 |
