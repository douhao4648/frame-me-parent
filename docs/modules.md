...
- **可配置项**：sa-token 原生参数（`token-name` / `timeout` / `active-timeout` / `is-concurrent` / `is-share` / `cookie.*` 等）走官方 `sa-token.*` 配置路径（秒数 long 形式，见 sa-token 官方文档），由官方 starter 的 `SaBeanRegister` 绑定；本模块 `me.auth.sa-token.*` 仅承载框架自有配置（默认值以 `SaTokenAuthProperties` 源码为准）：
  ...
  - `me.auth.sa-token.logic-type` — 账号体系标识（sa-token 多账号 loginType），默认 `login`（即 `StpUtil` 默认体系）。配置为非默认值（如 SSO 的 `sso`）时，本模块全部认证动作（登录/登出/验 token/路径规则校验）改走 `SaManager.getStpLogic(logicType)`，Redis key 变为 `{tokenName}:{logicType}:token|session:*`，与默认 `login` 体系在同一 Redis 中共存隔离；请求头/Cookie 名仍取全局 `sa-token.token-name`。业务侧 `@SaCheck*` 注解需同步加 `type` 属性指向同一体系（装配期解析即完成 SaManager 注册，注解按 type 查找可用）。
  ...
  - `me.auth.sa-token.max-lifetime` — 会话绝对寿命上限（秒），默认 `604800`（7 天），`<= 0` 表示不限制。登录时把时间戳记入 Account-Session，滑动续期超过上限即拒绝（4001）强制重新登录，防止被偷 token 无限续命；存量会话无时间戳时补记当前时间（一次宽限）。
...
`frame-me-launcher/frame-me-sso` 是独立可运行的 Spring Boot 认证服务，聚合 `frame-me-sso-api`（契约）+ `frame-me-sso-service`（启动服务）+ `frame-me-sso-starter`（下游接入 starter）。提供应用注册表、授权码 + client_credentials 双模式颁发 sa-token 不透明 token、/userinfo 代验、用户 CRUD、事件踢人（档3）。接入指南见 `docs/guides/sso.md`。
...
| `frame-me-sso-starter` | 下游应用一键接入 SSO 的 starter：注册 SSO HTTP Interface 客户端代理（`IAuthApi`/`IUserApi`/`IAppApi`，group=`sso`）+ `@Import(UserLogoutEventConfiguration.class)` 订阅踢人事件；**RP 全家桶**：`SsoAuthAutoConfiguration` 装配 `SsoAuthController`+`SsoAuthService`+`SsoLoginFlowController`+`SsoStateStore`+`SsoLogoutEventListener`（+ `SsoAuthUserDetailsServiceImpl` 兜底）。下游引本 starter + auth-sa-token/auth-jwt 即开箱获得 `POST /api/auth/sso-login` 端点 + `GET /sso-authorize` 登录发起（签发会话绑定的一次性 state，防登录 CSRF）+ 回调落地两端点（`GET /index` SPA hash 回调 / `GET /callback` 服务端 Cookie 会话回调，均原子消费 state）+ 踢人监听。强依赖 `frame-me-starter-auth`；登录端点（`loginByUser`）与踢人监听（`logoutByUserId`）均走 SPI，sa-token/JWT 两套实现都覆盖 |
...
| `SsoAuthAutoConfiguration` | `sso-starter/config/SsoAuthAutoConfiguration` | RP 装配：`@Import(SsoAuthService/SsoAuthController/SsoLoginFlowController/SsoLogoutEventListener)` + `@ConditionalOnMissingBean` 兜底装配 `SsoAuthUserDetailsServiceImpl`（下游自定义 `IAuthUserDetailsService` 时自动退让）；`@ConditionalOnClass(IAuthService)` + `me.sso.client.enabled` 开关；登录端点与踢人监听均走 SPI，sa-token/JWT 两套通用 |
| `SsoLoginFlowController` | `sso-starter/auth/SsoLoginFlowController` + `resources/sso/index.html` | 登录发起与回调落地三端点（类级 `@Anonymous`）：**发起** `GET /sso-authorize?target=<站内地址>`（`me.sso.client.authorize-path` 可配）——签发高熵一次性 state（`SsoStateStore`，Redis 存 state→target、Cookie nonce 绑定浏览器，默认 10 分钟过期）后 302 到 SSO authorize，登录必须从该端点发起、调用方不能自带 state。**方式 A** `GET /index`（`me.sso.client.index-path` 可配）SPA 回调——带 code 时服务端 GETDEL 原子消费 state + 比对 nonce Cookie（伪造/过期/重放/跨浏览器一律 4001），取出绑定的 target，302 到 `target#code=<code>`（hash 不进服务端日志/Referer），目标页从 `location.hash` 取 code 调 `/sso-login`；不带 code 的直达访问返回静态占位页（防自转循环）；HTML 以 `Resource` 形式返回（不走 `static/`：避免劫持下游 `/` welcome page 与静态资源互踩，且 `@Anonymous` 可随 starter 分发、免配 whitelist）。**方式 B** `GET /callback`（`me.sso.client.callback-path` 可配）服务端回调——原子消费 state 取出 target，code 由服务端直接换 token 建本地会话（sa-token 原生写 Cookie）后 302 回跳 target（保留 hash 片段、非法字符按需 percent-encode）；浏览器同站 Cookie 会话场景前端零 JS，JWT 下游不适用（详见 `docs/guides/sso.md` 回调落地两种方式） |
...
| `SsoLogoutEventListener` | `sso-starter/auth/SsoLogoutEventListener` | `@EventListener(UserLogoutEvent)`：调 `IAuthService.logoutByUserId` 清本地会话（走 SPI，sa-token 清 sa-token 会话，JWT 删 Refresh Token，两套通用） |
...
`frame-me-launcher/frame-me-audit` 是审计中心聚合工程，聚合 `frame-me-audit-api`（契约）+ `frame-me-audit-service`（启动服务）。定位：订阅 `audit:log` 事件通道，接收 `frame-me-starter-op-audit` 桥接来的审计事件并持久化到 MySQL；同时作为 SSO 下游（RP），用户通过 SSO 授权码登录后建 audit 自己的 sa-token 本地会话。

### 模块职责

| 模块 | 职责 |
|---|---|
| `frame-me-audit-api` | 审计 API 契约模块：`ILogApi`（审计日志管理查询）+ `LogQuery`/`LogVO`；`SsoAuthController` 的 sso-login 端点已搬至 `frame-me-sso-starter`，后续按需抽 `ISsoAuthApi` 契约 |
| `frame-me-audit-service` | 启动服务：订阅 `audit:log` 持久化 + 审计日志管理查询（`LogController`）+ SSO RP sa-token 会话治理 |

### 关键类

| 类 | 路径 | 职责 |
|---|---|---|
| `Application` | `audit/Application` | 主启动类，`@Import(AuditLogEventConfiguration.class)` 订阅 `audit:log` 通道 |
...
### 可配置项（`me.audit.*` / `me.sso.client.*`）

| 属性 | 默认值 | 说明 |
|---|---|---|
| `me.auth.sa-token.path` | `/api/auth` | sa-token 认证接口基础路径（`SaTokenAuthController` 的 /login /logout /refresh /user） |
| `me.auth.sa-token.max-lifetime` | `604800` | 会话绝对寿命上限（秒），滑动续期超过即拒绝（4001）；`<= 0` 不限制 |
| `me.sso.client.path` | `/api/auth` | SSO RP 登录端点基础路径（`SsoAuthController` 的 /sso-login，来自 sso-starter，自治不绑 sa-token）；**改 path 时需与 `me.auth.sa-token.path` 同步配**，确保端点共路径 |
...
- `auth.IAppAuthenticator` + `ConfigAppAuthenticator` — 配置版 app 鉴权：`Authorization: Signature` + `Date` 头（对齐 APISIX hmac-auth / draft-cavage，HMAC-SHA256 + 300s 时钟偏差容差防重放 + 常量时间比较），可选 `x-nonce` 声明后 Redis SET NX 一次性消费（精确防重放，无 Redis/故障 fail-closed），认证后注入 `X-App-Key`。
