...
frame-me-api  →  frame-me-starter-base  →  frame-me-starter-adapter   (可被外部项目重写)
                   ...
                   frame-me-starter-cloud  ────────────────────┘   │
                         ↓                            │   │
                      frame-me-booter ---------------------------------┘
                      （聚合启动模块，供外部 xx-service 引用，不含 adapter）
```
...
- `frame-me-booter`：聚合启动模块，依赖 `frame-me-starter-auth`、`frame-me-starter-cloud`，本身无业务源码与自动装配，供外部业务工程的 `xx-service` 统一引入通用能力。`frame-me-starter-adapter` 不纳入聚合。
- `frame-me-tester`：可运行入口，依赖 `frame-me-starter-base` 与 `frame-me-starter-adapter`。
...
- `frame-me-starter-base` 注册 `com.frame.me.base.mybatis.config.MybatisPlusConfiguration`
  - 注册 Bean：`MybatisPlusInterceptor`（含分页插件、乐观锁插件）、`BaseMetaObjectHandler`（需配置开启）。
...
`frame-me-booter` 作为聚合模块没有自己的自动装配类，它通过传递依赖自动引入 `frame-me-starter-base` 的自动配置。`frame-me-starter-adapter` 不纳入 `frame-me-booter`，因此外部项目可选择使用默认适配层或自行实现。
...
- `FrameBaseMapper`：`frame-me-starter-base/src/main/java/com/frame/me/base/mybatis/mapper/FrameBaseMapper.java`
