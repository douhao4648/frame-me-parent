...
public interface IAppApi {
    ...
    IResult<Boolean> updateApp(@Parameter(description = "应用 ID", required = true) @PathVariable String appId,
                               @RequestBody AppUpdateDTO app);

    /**
     * 重置应用密钥（仅 EXTERNAL，子动作）.
     */
    @Operation(summary = "重置密钥", description = "重置 EXTERNAL 应用密钥，明文仅此一次返回")
     ...
     * 禁用应用（状态变更子动作，非物理删除）.
     */
    @Operation(summary = "禁用应用", description = "禁用应用，已颁发 token 自然过期")
