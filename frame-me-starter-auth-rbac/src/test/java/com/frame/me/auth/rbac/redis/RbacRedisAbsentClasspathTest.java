...
/**
 * RedisClientRegistry 缺席时的装配测试.
 *
 * <p>multi-redis 是 auth-rbac 的 optional 依赖，消费方未引入时 {@link RbacRedisAutoConfiguration}
 * 必须在 ASM 元数据评估阶段整体退避（不加载 {@code RedisClientRegistry}、不抛 NCDFE）,
...
class RbacRedisAbsentClasspathTest {
    ...
    /**
     * RedisClientRegistry 缺席：redis 配置退避，配置版 provider 生效，零 redis bean.
     * 若上下文因 NCDFE 启动失败，以下 bean 断言会连带启动失败信息一起报出。
     */
    @Test
    void redisClientRegistryAbsent_redisAutoConfigBacksOff_configProviderActive() {
        runner.run(context -> {
            ...
            // 数据源插槽由始终在场的 RbacAutoConfiguration 注册，不受 RedisClientRegistry 缺席影响
